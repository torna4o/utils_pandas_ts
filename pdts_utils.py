...
def max_pickler(serie):
    """
    The aim is picking the largest valid number chunks
    not NA filled previously.
    
    Running valid_chunk and nan_chunk will give us
    the exact length and location of that continuous data part.
    """
    locator = 0
    pos_chunk, len_pos, key = valid_chunk(serie)
    print(pos_chunk)
    print(pos_chunk.index(len_pos))
    print(key)
    na_chunk = nan_chunk(serie)
    
    if key == True:
        if pos_chunk.index(len_pos) != 0:
            locator = np.array(pos_chunk)[:pos_chunk.index(len_pos)].sum() + np.array(na_chunk)[:pos_chunk.index(len_pos)].sum()
        elif pos_chunk.index(len_pos) == 0:
            locator = 0
    elif key == None:
        if pos_chunk.index(len_pos) !=0:
            locator = np.array(pos_chunk)[:pos_chunk.index(len_pos)].sum() + np.array(na_chunk)[:(pos_chunk.index(len_pos))+1].sum()
        elif pos_chunk.index(len_pos) == 0:
            locator = na_chunk[0]
    print("Start point for the longest valid chunk: ", locator)
    print("End point for the longest valid chunk: ", locator + len_pos)
    
    return locator, (locator + len_pos)